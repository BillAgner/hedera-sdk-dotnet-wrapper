...
namespace hederasdkdotnetwrapper {
	...
	public:
		...
		static HederaSecretKey HederaSecretKeyGenerate();
		...
		static HederaError HederaSecretKeyFromStr(const char* s, HederaSecretKey* out);
		...
		static char* HederaSecretKeyToStr(HederaSecretKey*);
		
		/// Derive a [HederaPublicKey] from a [HederaSecretKey].
		static HederaPublicKey HederaPublicKeyFromSecretKey(HederaSecretKey*);

		/// Format a [HederaPublicKey] as a hex-encoded string of the secret key encoded with a PKIX wrapper (
		/// defined in RFC 3280).
		///
		/// Returns ownership of the string. Must be freed with [free].
		static char* HederaPublicKeyToStr(HederaPublicKey*);
	};
}
